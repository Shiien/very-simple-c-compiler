...
{
    if (node->nodeType == TreeNode::NODE_STATMENT && static_cast<StatementNode *>(node)->type == StatementNode::ST_SCOPE)
    {
        node = node->child;
    }
    while (node)
...
void handleFuncCall(TreeNode *node)
{
    ...
    //cerr << "shi bu shi zai zhe li " << children.size() << endl;
...
string handleExprNode(TreeNode *node)
{
    ...
    if (node->nodeType == TreeNode::NODE_OPERATOR)
    {
        ...
        cerr << OperatorNode::getTypeStr(opType) << endl;
        ...
        cerr << "++++ operator not handled " << OperatorNode::getTypeStr(opType) << endl;
        return getSymbolMemory(ownSymbol);
    }
...
void handleFunctionNode(TreeNode *node)
{
    ...
    if (children->nodeType == TreeNode::NODE_STATMENT && static_cast<StatementNode *>(children)->type == StatementNode::ST_RET)
    {
    }
    else
    {
...
void generateCode(TreeNode *node)
{
    ...
    if (node->nodeType == TreeNode::NODE_STATMENT)
    {
        ...
        if (stnode->type == StatementNode::ST_IF_ELSE || stnode->type == StatementNode::ST_IF)
        {
            handleIf(stnode);
        }
    ...
    else if (node->nodeType == TreeNode::NODE_OPERATOR || node->nodeType == TreeNode::NODE_EXPRESSION)
    {
        handleExprNode(node);
    }
